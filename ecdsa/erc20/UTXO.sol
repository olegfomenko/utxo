...
contract UTXOERC20 is IUTXO {
    ...
    function deposit(address _token, uint256 _amount, OUTPUT[] memory _outs) public override {
        ...
        for (uint _i = 0; _i < _outs.length; _i++) {
            UTXO memory _utxo = UTXO(_token, _outs[_i]._amount, _outs[_i]._owner, false);
            utxos.push(_utxo);
    ...
    function getOutAmount(OUTPUT[] memory _outs)  internal pure returns (uint256) {
        uint256 result = 0;
        for (uint _i = 0; _i < _outs.length; _i++) {
    ...
    function withdraw(INPUT memory _input, address _to) public override {
        ...
        require(_utxo._owner == keccak256(_data).recover(_input._signature), "invalid signature");
    ...
    function transfer(INPUT[] memory _inputs, OUTPUT[] memory _outputs) public override {
        ...
        for(uint _i = 0; _i < _outputs.length; _i++) {
            ...
            _data = abi.encodePacked(_data, _outputs[_i]._amount, _outputs[_i]._owner);
        }
        ...
        for(uint _i = 0; _i < _inputs.length; _i++) {
            ...
            require(_token == _utxo._token, "all UTXO should be for the same token");
            require(!_utxo._spent, "UTXO has been spent");
            require(_utxo._owner == keccak256(abi.encodePacked(_inputs[_i]._id, _data)).recover(_inputs[_i]._signature), "invalid signature");
        ...
        uint256 _id = utxos.length;
        for (uint _i = 0; _i < _outputs.length; _i++) {
            UTXO memory _newUtxo = UTXO(_token, _outputs[_i]._amount, _outputs[_i]._owner, false);
            utxos.push(_newUtxo);
            emit UTXOCreated(_id++, msg.sender);
        }
    }
